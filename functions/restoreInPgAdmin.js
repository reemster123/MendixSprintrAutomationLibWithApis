...
module.exports = async (cmdArguments, filePath) => {
    try {
        ...
        // click on databasesFolder in explorer
        /*console.log('**Starting selectDatabases functionality**');
        const databasesFolder = await page.waitForSelector('div[tabindex="-1"][role="treeitem"][aria-selected="true"][class="aciTreeLine"][aria-level="3"]', { visible: true });
        console.log('**Found databases Folder**');
        await databasesFolder.click();
        console.log('**Clicked databases Folder**'); */
